...
export default function Home({ user, cart, cartCount, addToCart, setCart, selectedStore, setSelectedStore, logout }) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setProductList(allP.filter(x => !x.country || x.country === userCountry));
      const myStores = allS.filter(x => !x.country || x.country === userCountry);
      setStoreList(myStores);
      if (myStores.length > 0 && !selectedStore) setSelectedStore(myStores[0]);
    ...
    <div style={{display:'flex',flexDirection:'column',flex:1}}>
      ...
      <div style={{overflowY:'auto',flex:1,padding:14}}>
        {cart.length===0?<div style={{textAlign:'center',padding:60,color:'#9C8878'}}><div style={{fontSize:48}}>🛒</div><div style={{fontWeight:600,marginTop:8}}>Cart is empty</div><button onClick={()=>setScreen('home')} style={{marginTop:16,padding:'10px 24px',background:'#C8541A',color:'#fff',border:'none',borderRadius:12,fontSize:14,fontWeight:700,cursor:'pointer'}}>Browse Products</button></div>:(<>
          ...
          <button onClick={async()=>{if(!selectedStore)return showToast('Select a store first');try{const items=cart.map(i=>({product_id:i.id,quantity:i.qty,unit_price:Number(i.retail_price),wholesaler_id:null}));await orders.create({buyer_id:user.id,store_id:selectedStore.id,items});setCart([]);showToast('Order placed!');setScreen('orders');}catch(e){showToast('Error: ' + (e.response?.data?.error || e.message || 'Unknown error'));}}} style={{width:'100%',height:52,background:'#C8541A',color:'#fff',border:'none',borderRadius:14,fontSize:16,fontWeight:700,cursor:'pointer'}}>Confirm Order · {formatPrice(total,userCountry)} →</button>
        </>)}
      </div>
      <Nav/>
    </div>
